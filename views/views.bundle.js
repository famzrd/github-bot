...
function eventHandler(event) {
  ...
  if (event.target.dataset.action === 'close-all') {
    browser.tabs.query({
      ...
      browser.tabs.remove(tabs.map(function (i) {
        return i.id;
      }));
    });
  }
...
function createOptions(node, identity) {
  for (var _i = 0, _arr = ['Create', 'Close All']; _i < _arr.length; _i++) {
    var option = _arr[_i];
...
if (browser.contextualIdentities === undefined) {
  ...
  browser.contextualIdentities.query({}).then(function (identities) {
    ...
    try {
      for (var _iterator = identities[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        var identity = _step.value;
        var row = document.createElement('div');
        var span = document.createElement('span');
        span.className = 'identity';
        span.innerText = identity.name;
        span.style = "color: ".concat(identity.color);
